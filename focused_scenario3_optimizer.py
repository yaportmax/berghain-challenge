#!/usr/bin/env python3
"""
Focused optimization strategy for scenario 3 prioritizing the 3 rarest traits.

Strategy per user requirements:
1. Focus on queer_friendly, vinyl_collector, and german_speaker (3 rarest traits)
2. Only accept people who have at least one of those 3 traits (until capped)
...
class FocusedScenario3Optimizer(BerghainBouncer):
    """Focused optimizer prioritizing the 3 rarest traits for scenario 3."""
    ...
    def _should_accept_focused_strategy(
        self, 
        attrs: Dict[str, bool], 
        ...
        """
        Accept people who have at least one active priority trait.
        Once all priority traits are capped, accept anyone to fill remaining constraints.
        """
    ...
    def run_focused_strategy(self) -> Dict:
        """Run scenario 3 with focused strategy on 3 rarest traits."""
        print("=== Focused Scenario 3 Strategy ===")
        print("Priority traits: queer_friendly, vinyl_collector, german_speaker")
        print("Strategy: Only accept people with at least one priority trait (until capped)")
        print()
        ...
        while result.get("status") == "running":
            ...
            accept = self._should_accept_focused_strategy(attrs, counts, admitted)
        ...
        return {
            "strategy": "focused_3_rarest_traits",
            "status": final_status,
    ...
    def save_results(self, result: Dict, filename: str = "focused_optimization_result.json"):
        """Save optimization result to file."""
        output = {
            "strategy_description": "Focus on 3 rarest traits: queer_friendly, vinyl_collector, german_speaker",
...
def main():
    optimizer = FocusedScenario3Optimizer()
    result = optimizer.run_focused_strategy()
    
    output = optimizer.save_results(result)
    
    print("\n=== FOCUSED STRATEGY RESULTS ===")
