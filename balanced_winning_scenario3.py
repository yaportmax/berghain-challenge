#!/usr/bin/env python3
"""
Balanced winning strategy for scenario 3 that ensures all constraints are met.

Strategy per user requirements:
1. Focus on queer_friendly, vinyl_collector, and german_speaker (3 rarest traits)
2. Accept people with at least one of those 3 traits (until each is capped)
3. Once a trait is capped, stop factoring it into targeting
4. BUT also accept people who help with severely unmet constraints to ensure scenario completion
...
class BalancedWinningScenario3(BerghainBouncer):
    """Balanced winning optimizer that prioritizes rare traits but ensures scenario completion."""
    ...
    def _should_accept_balanced_strategy(
        self, 
        attrs: Dict[str, bool], 
        ...
        """
        Balanced strategy:
        1. Always accept people with active priority traits
        2. Accept people who help with any unmet constraint (to ensure scenario completion)
        3. Use the priority traits as a preference, not a strict filter
        """
    ...
    def run_balanced_strategy(self) -> Dict:
        """Run scenario 3 with balanced strategy."""
        print("=== Balanced Winning Scenario 3 Strategy ===")
        print("Priority traits: queer_friendly, vinyl_collector, german_speaker")
        print("Strategy: Prioritize rare traits but accept anyone who helps with unmet constraints")
        print()
        ...
        while result.get("status") == "running":
            ...
            accept = self._should_accept_balanced_strategy(attrs, counts, admitted)
        ...
        return {
            "strategy": "balanced_priority_traits_with_completion",
            "status": final_status,
...
def main():
    optimizer = BalancedWinningScenario3()
    result = optimizer.run_balanced_strategy()
    
    print("\n=== BALANCED WINNING STRATEGY RESULTS ===")
