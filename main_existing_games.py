...
def main():
    ...
    print("Using existing stale games to avoid rate limits...\n")
    
    existing_games = {
        1: "p5Wr9bXfdrw",  # Scenario 1: 0/1000, Stale 8m ago
        2: "9x0AuplPriU",  # Scenario 2: 0/1000, Stale 8m ago  
        3: "7PHYP_S1e2Y"   # Scenario 3: 0/1000, Stale 8m ago
    }
    
    for scenario in [1, 2, 3]:
        print(f"Starting Scenario {scenario}...")
        try:
            game_id = existing_games[scenario]
            result = bouncer.run_scenario_with_existing_game(game_id, scenario)
    ...
    with open("results_existing_games.json", "w") as f:
        json.dump({
    ...
    print(f"\nDetailed results saved to results_existing_games.json")
